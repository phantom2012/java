...
<c:if test="${!empty historyGroups}">
    <nav class="navA">
        <c:forEach var="hisGroupInfo" items="${historyGroups}">
            <a href="http://tvbar.yy.com/tvbar/groupPage.action?groupId=${hisGroupInfo.groupId}"><span>${hisGroupInfo.title}</span></a>
        </c:forEach>
    </nav>
</c:if>
    ...
    <div id="carousel">
        <c:forEach var="recGroupInfo" items="${recommendGroups}">
            <div class="card"><img src="${recGroupInfo.headIconUrl}" alt="" data-name="${recGroupInfo.title}"
                                   data-url="http://tvbar.yy.com/tvbar/groupPage.action?groupId=${recGroupInfo.groupId}&uid=${uid}">
            </div>
        </c:forEach>
    </div>
