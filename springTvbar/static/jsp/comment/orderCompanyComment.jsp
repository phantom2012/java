            ...
            <div class="grade-con">
                ...
                <div>
                   <@if(item.star_comment != ''){@>
                     <p>对方留言：<@=item.star_comment@></p>
                   <@}@>
                   <@if(item.agency_comment != ''){@>
                     <p>我的回复：<@=item.agency_comment@></p>
                   <@}@>
                   <@if(item.add_comment != ''){@>
                     <p>对方留言：<@=item.add_comment@></p>
                   <@}@>
                   <@if(item.agency_add_comment != ''){@>
                     <p>我的回复：<@=item.agency_add_comment@></p>
                   <@}@>
                </div>                           
                <p class="leave-message">留言：<textarea id="commentText" name="" <@if(item.isTextDisabled){@> disabled="disabled" <@}@> ></textarea></p>
                <p class="btn-con"><a href="javascript:void(0)" onclick="orderComment.appendAgencyComment()" class="btn orange"><@=item.buttonText@></a></p>
            </div>
        </div>
    </div>
</script>
